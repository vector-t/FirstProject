...
static public class NGUIRichText
{
	...
	static public string StripSymbols (string text)
	{
		if (text != null)
		{
			for (int i = 0, imax = text.Length; i < imax; )
			{
				...
				{
					int index = i;
					if (GetCustomSymbol (text, ref index) != null) {
						text = text.Remove (i, index);
						text = text.Insert (i, "吴");
						imax = text.Length;
						continue;
					}
				}
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false)
	{
		...
		for (; offset < textLength; ++offset)
		{
            ...
            if(customSymbol != null)
            {
                glyphWidth = finalSpacingX + fontSize * fontScale;
            }
            else if(symbol != null)
            {
